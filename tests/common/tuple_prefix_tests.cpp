...
TEST(tuple_prefix, nonempty_prefixes)
{
    ...
    static_assert(std::is_same_v<tTuplePrefix<T, 1>, std::tuple<int>>);
    static_assert(std::is_same_v<tTuplePrefix<T, 2>, std::tuple<int, double>>);
    static_assert(std::is_same_v<tTuplePrefix<T, 3>, std::tuple<int, double, std::string>>);
    static_assert(std::is_same_v<tTuplePrefix<T, 4>, std::tuple<int, double, std::string, char>>);
}

TEST(tuple_prefix, empty_prefix)
{
    using T = std::tuple<int, double, std::string, char>;
    static_assert(std::is_same_v<tTuplePrefix<T, 0>, std::tuple<>>);
}
...
TEST(applyHead, full)
{
    ...
    applyHead<4>(f4, std::make_tuple(1, 2.5, "alma", 'x'));
    EXPECT_EQ(testedA, 1);
    EXPECT_DOUBLE_EQ(testedB, 2.5);
    EXPECT_EQ(testedC, "alma");
    EXPECT_EQ(testedD, 'x');
...
TEST(applyHead, partial)
{
    ...
    applyHead<2>(f2, std::make_tuple(1, 2.5, "alma", 'x'));
    EXPECT_EQ(testedA, 1);
    EXPECT_DOUBLE_EQ(testedB, 2.5);
    EXPECT_EQ(testedC, "");
    EXPECT_EQ(testedD, 0);
...
TEST(applyHead, empty)
{
    ...
    applyHead<0>(f0, std::make_tuple(1, 2.5, "alma", 'x'));
    EXPECT_EQ(testedA, 0);
    EXPECT_DOUBLE_EQ(testedB, 0.0);
    EXPECT_EQ(testedC, "");
    EXPECT_EQ(testedD, 0);
